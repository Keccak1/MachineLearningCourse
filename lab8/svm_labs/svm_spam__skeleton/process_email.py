...
def process_email(email_contents: str) -> List[int]:
    ...
    for token in tokens:
        ...
        def findWord(dictionary, search_word):
            return [index for index,word in dictionary.items() if word == search_word]


        value = findWord(vocabulary_dict, token)
        if(value):
            word_indices.append(value[0])
